...
class ViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, MPCManagerDelegate {
    ...
    //MODIFIQUEI
    @IBAction func matchWithSelectedPeers(sender: AnyObject) {
        
        var selectedPeers = [MCPeerID]();
        
        if let indexArray = tblPeers.indexPathsForSelectedRows()?{
        
            for var i=0; i<indexArray.count; ++i{
                selectedPeers.append(appDelegate.mpcManager.foundPeers[indexArray[i].row] as MCPeerID)
            }
